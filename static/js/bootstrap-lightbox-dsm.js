 ...
 *
 * Modified by Matthieu Morin <morinmatthieu@gmail.com>
 */
+function ($) {
  'use strict';
  ...
  var Lightbox = function (element, options) {
	...
	// We set the src of the img instead of an AJAX load.
    if (this.options.remote) {
      this.$element
        .find('.modal-content > img')
        .attr('src', this.options.remote)
    }
  }
  ...
  Lightbox.prototype.removeBackdrop = function () {
	...
	Lightbox.prototype.preloadSize = function(callback)
	{
		...
		padTop    = parseInt( that.$element.find('.modal-content').css('padding-top')    , 10);
		padBottom = parseInt( that.$element.find('.modal-content').css('padding-bottom') , 10);
		padLeft   = parseInt( that.$element.find('.modal-content').css('padding-left')   , 10);
		padRight  = parseInt( that.$element.find('.modal-content').css('padding-right')  , 10);

		// Load the image, we have to do this because if the image isn't already loaded we get a bad size
		$image    = that.$element.find('.modal-content').find('img:first');
		...
		{		
			...
			that.$element.find('.modal-content').css({
				'width': preloader.width + padLeft + padRight,
				'height': preloader.height + padTop + padBottom
			});
			$image.css({
  ...
  $(document)
    .on('show.bs.lightbox', '.modal', function () { $(document.body).addClass('modal-open') })
    .on('hidden.bs.lightbox', '.modal', function () { $(document.body).removeClass('modal-open') })
