...
def index(request):
    ...
    return render_to_response('settings.html', {
        ...
        'vesselform': forms.VesselForm(
            instance=models.Vessel.objects.latest('id')
            ),
        'userform': forms.UserForm(
            instance=models.User.objects.get(id=request.user.id)
            ),
        },
...
def import_data(request):
    ...
    if request.method == 'POST' and request.is_ajax():
        f = forms.ImportForm(request.POST, request.FILES)
        if f.is_valid():  # All validation rules pass
            ...
            # Returning errors
            errors = dict(
                [(k, [unicode(e) for e in v]) for k, v in f.errors.items()]
            )
...
def import_key(request):
    ...
    if request.method == 'POST' and request.is_ajax():
        f = forms.ImportKeyForm(request.POST, request.FILES)
        if f.is_valid():  # All validation rules pass
            # Process the data in form.cleaned_data
            import_file = request.FILES['file_obj']
            # Passing the file to the core function
...
def user(request):
    ...
    if request.method == 'POST' and request.is_ajax():
        return settings_validation(request,
                                   forms.UserForm,
                                   models.User.objects.get(id=request.user.id)
                                   )
...
def vessel(request):
    ...
    if request.method == 'POST' and request.is_ajax():
        return settings_validation(request,
                                   forms.VesselForm,
                                   models.Vessel.objects.latest('id')
                                   )
