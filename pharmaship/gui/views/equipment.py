...
class View:
    ...
    def toggle_article(self, source, grid, equipment, row_num):
        ...
        for article in articles:
            ...
            if article["location"]["rescue_bag"]:
...
def get_packing_text(article, params):
    ...
    if data["id"] >= 20:
        ...
        template = _("{quantity} {packing_name} of {packing_content} {packaging}")

    quantity = article["quantity"]/data["content"]

    plural_packing = pluralizer.pluralize(
        data["name"],
        quantity
        )
    plural_packaging = pluralizer.pluralize(
        first_lower(article["equipment"]["packaging"]),
        data["content"]
        )
